...
void read_vector() {
  ...
  infile.close();
  cout <<"l2 - norm ^2 of input test data = " << sum_check << endl;
}
...
int main(){
  ...
  */
  ...
  //vector<vector<int64_t> > ct=RLWE64_Enc_sk(input,x,p1,q1);     

  //gettimeofday(&tend,NULL);
  //double  time_enc = tend.tv_sec-tstart.tv_sec+(tend.tv_usec-tstart.tv_usec)/1000000.0;
  //cout <<"Total time for encryption: "<<time_enc<<"s. "<<endl;
  ...
  fout.close();
  cout<<"Encrypt the input and store the encrypted feature vector. "<<endl;
...
/*
  ...
  for (int i = 0; i < l1; ++i)
  {
    ip += test_vector[i]*test_vector[i];
  }
  cout <<ip <<endl;
*/  
